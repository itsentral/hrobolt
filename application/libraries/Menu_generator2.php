...
class Menu_generator
{
	...
	public function build_menus($type=1)
	{
		...
		if($type==1)
		{
			...
			$html = "<ul class='sidebar-menu bg-yellow'>
							<li class='header bg-blue'></li>
	                        <li class='".check_class('dashboard', TRUE)." bg-blue'>
	                            <a  href='".site_url()."'>
			...
			if(is_array($menu) && count($menu))
			{
				foreach ($menu as $rw) {
					...
					$html .= "
            			  <li class='treeview {$active} bg-blue'>
                      <a href='#'>
                        <i class='".$icon."'></i>
					...
					foreach ($submenu as $sub) {
						...
						if(count($submenusub) == 0)
						{
							if($sublink !="#")
							{
								...
								$html .= "
								<li class='".$active."'>
									<a class='bg-yellow' href='".($sublink == '#' ? '#' : site_url($sublink))."'"." ".$subtarget.">
...
*/
	...
	public function build_menus_OLD($type=1)
	{
		...
		if($type==1)
		{
			...
			$html = "<ul class='sidebar-menu bg-blue'>
							<li class='header'></li>
