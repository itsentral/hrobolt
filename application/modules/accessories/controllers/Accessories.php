...
{
    //Permission
    protected $viewPermission 	= 'Master_Barang_Stock.View';
    protected $addPermission  	= 'Master_Barang_Stock.Add';
    protected $managePermission = 'Master_Barang_Stock.Manage';
    protected $deletePermission = 'Master_Barang_Stock.Delete';
    ...
    public function getDataFinsihGoods($id) 
    {
      $accessories = $this->db->query("SELECT * FROM accessories_category WHERE id = $id")->row();

      if ($accessories->nm_category == 'Finish Goods') {
        $dataFinishGoods = $this->db->query("SELECT * FROM ms_inventory_category3 WHERE aktif = 'aktif' AND deleted = 0")->result();
      }

      if (isset($dataFinishGoods)) {
        $data = [
          'code' => 200,
          'status' => 'OK',
          'message' => 'Berhasil Ambil Data',
          'data' => $dataFinishGoods
        ];
      } else {
        $data = [
          'code' => 404,
    ...
    public function getDataTheFinishGood($id) 
    {
      $dataFinishGoods = $this->db->query("SELECT ms3.*, ms1.nama AS brand FROM ms_inventory_category3 ms3 JOIN ms_inventory_category1 ms1 ON ms1.id = ms3.id_category1 WHERE ms3.id = $id AND ms3.aktif = 'aktif' AND ms3.deleted = 0")->row();
    ...
    private function getNameFinishGood($id)
    {
      $data = $this->db->query("SELECT nama FROM ms_inventory_category3 WHERE id = $id")->row();
    ...
    public function add()
    {
      if($this->input->post()){
        ...
        $id_gudang        = $data['gudang_id'];
        $stock_name       = ($data['stock_name']) ? $data['stock_name'] : $this->getNameFinishGood($data['stock_name_finish_goods']);
        ...
        $warehouses 		  = $this->db->get_where('warehouse', array('status' => 'Aktif'))->result();
