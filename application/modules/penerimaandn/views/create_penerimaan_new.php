...
<div class="box box-default ">
	<div class="box-header">
		<div class="nav-tabs-custom">
			...
			<li class="add"><a href="#" data-toggle="tab" id="tambah2">Add Debit Note</a></li>
            ...
            <table class="table table-bordered" width="100%" id="tabel-detail-mutasi">
				<thead>
					<tr class="bg-blue">
						...
						<th class="text-center">Total DN</th>
						<th class="text-center">Sisa DN</th>
  ...
  <div class="box active">
    <div class="box-body">
		<div class="row">
			...
			<div class="col-lg-6">
					    <div class="form-group ">
                            <label class="col-sm-4 control-label">Total Bayar DN</label>
                            <div class="col-sm-6">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel"><span class="fa fa-file-pdf-o"></span>&nbsp;Data DN</h4>
      </div>
      <div class="modal-body" id="MyModalBodyStok" style="background: #FFF !important;color:#000 !important;">
          <table class="table table-bordered" width="100%" id="list_item_stok">
              ...
              <tbody>
                  ...
                  $invoice = $this->db->query("SELECT * FROM tr_debit_note WHERE id_customer ='$cust'")->result();
				  if($invoice){
                  ...
                  ?>
						  <tr>
							  ...
							  <td><center><?php echo number_format($vs->total_tagihan) ?></center></td>
							  <td><center><?php echo number_format($vs->sisa_tagihan) ?></center></td>
							  <td>
								<center>
									<button id="btn-<?php echo $vs->no_pr?>" class="btn btn-warning btn-sm" type="button" onclick="startmutasi('<?php echo $vs->no_pr?>','<?php echo $vs->nm_customer?>','<?php echo $vs->total_tagihan?>','<?php echo $vs->sisa_tagihan?>')">
										Pilih
									</button>
								</center>
							  </td>
	 ...
	 function savemutasi(){
        ...
        function(isConfirm){
			if(isConfirm) {
				...
				$.ajax({
					url: siteurl+"penerimaandn/save_penerimaan",
					dataType : "json",
					type: 'POST',
					data: formdata,
...
//		totalterima();
    ...
    $(document).on('click', '.add', function(){
		...
		$.ajax({
			type:'POST',
			url:siteurl+'penerimaandn/TambahInvoice/'+id_customer,
	...
	$(document).on('click', '#lebihbayar', function(){
		...
		$.ajax({
			type:'POST',
			 url:siteurl+'penerimaandn/TambahLebihBayar/'+id_customer,
			data:{'id_customer':id_customer},
			success:function(data){
	...
	$(document).on('click', '.lebih', function(){
		...
		$.ajax({
			type:'POST',
			 url:siteurl+'penerimaandn/lebihbayar',
			data:{'id_customer':id_customer},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
	...
	$(document).on('click', '.createunlocated', function(){
		...
		$.ajax({
			type:'POST',
			 url:siteurl+'penerimaandn/createunlocated',
			data:{'id_customer':id_customer},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
