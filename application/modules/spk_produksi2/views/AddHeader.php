										...
										<select id="id_material" name="id_material" class="form-control select" onchange="Caristock()" required>
											<option value="">--Pilih--</option>
												<?php foreach ($results['material'] as $material){
													...
													?>
											<option value="<?= $material->id_category3?>" <?=$selx;?>><?= ucfirst(strtolower($material->nama_category2))?>-<?= ucfirst(strtolower($material->nama))?>-<?= ucfirst(strtolower($material->hardness))?>-<?= ucfirst(strtolower($material->thickness))?>|<?= ucfirst(strtolower($material->maker))?></option>
												<?php } ?>
										</select>
								</div>
							</div>
