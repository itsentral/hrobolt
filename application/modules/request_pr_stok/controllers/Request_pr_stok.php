...
class Request_pr_stok extends Admin_Controller
{
  ...
  public function save_reorder_all()
  {
    ...
    foreach ($getraw_materials as $key => $value) {
      $SUM += $value['request_pack'];  // qty request
      $ArrSaveDetail[$key]['so_number'] = $so_number;
      $ArrSaveDetail[$key]['id_material'] = $value['id'];
      $ArrSaveDetail[$key]['propose_purchase'] = $value['request_pack']; // qty request
    }
  ...
  public function PrintH2()
  {
    ...
    $data['header'] = $this->db->query("SELECT a.*, b.nm_customer, b.alamat, c.name as country_name, d.nm_pic, d.hp, d.email_pic, b.fax 
    FROM material_planning_base_on_produksi as a 
    LEFT JOIN material_planning_base_on_produksi x ON x.so_number = a.so_number 
    LEFT JOIN master_customers b ON b.id_customer = a.id_customer 
    LEFT JOIN country_all c ON c.iso3 = b.country_code
    LEFT JOIN customer_pic d ON d.id_pic = b.id_pic 
    WHERE a.so_number = '" . $id . "' ")->result();

    $data['detail']  = $this->db->query("SELECT a.*, if(b.code IS NULL, e.id_stock, b.code) as code, 
    if(b.nama IS NULL, e.stock_name, b.nama) as nama, if(b.konversi IS NULL, if(e.konversi <= 0, 1, e.konversi), b.konversi) as konversi, 
    if(c.code IS NULL, f.code, c.code) as satuan, if(d.code IS NULL, g.code, d.code) as satuan_packing 
