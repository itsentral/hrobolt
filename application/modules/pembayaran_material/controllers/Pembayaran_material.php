...
*/
	...
	public function save_payment() {
		...
		$id_payment_paid = $this->Pembayaran_material_model->generate_id_payment_paid($kode_bank);
		...
		if($this->db->trans_status() === false) {
			...
			$hutang       = str_replace(',', '', $post['total_payment'])+str_replace(',', '', $detail['nilai_ppn']);
			$bankcharge   =str_replace(',', '', $post['bank_charge']);
			$bank_nilai   = str_replace(',', '', $post['payment_bank']);
			...
			foreach ($datajurnal1 as $rec) {
				...
				if($rec->parameter_no=="3"){
					$det_Jurnaltes1[] = array(
						'nomor' => $nomor_jurnal, 'tanggal' => $payment_date, 'tipe' => 'BUK', 'no_perkiraan' => $bank_coa, 'keterangan' => $no_request.'. '.$keterangan, 'no_request' => $id_payment_paid, 'kredit' => ($bankcharge), 'debet' => 0, 'no_reff' => $no_request, 'jenis_jurnal'=>$jenis_jurnal, 'nocust'=>$id_supplier, 'stspos' => '1'
					);
				}
				if($rec->parameter_no=="4"){
					$det_Jurnaltes1[] = array(
