  ...
  public function get_query_approval_pr_material_head($product, $costcenter, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
  {
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              (SELECT @row:=0) r
            WHERE 1=1 AND a.category in ('pr material','base on production') AND a.reject_status = '0' AND a.booking_date IS NOT NULL AND z.status_app = 'N' AND a.app_post IS NULL " . $costcenter_where . " " . $product_where . " AND (
  ...
  public function get_query_approval_pr_material_cost_control($product, $costcenter, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
  {
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              LEFT JOIN master_customers b ON a.id_customer=b.id_customer,
              (SELECT @row:=0) r
            WHERE 1=1 AND a.category in ('pr material','base on production') AND a.reject_status = '0' AND a.booking_date IS NOT NULL AND z.status_app = 'N' AND a.app_post = '2' " . $costcenter_where . " " . $product_where . " AND (
              b.nm_customer LIKE '%" . $this->db->escape_like_str($like_value) . "%'
              OR a.so_number LIKE '%" . $this->db->escape_like_str($like_value) . "%'
              OR a.project LIKE '%" . $this->db->escape_like_str($like_value) . "%'
              OR a.no_pr LIKE '%" . $this->db->escape_like_str($like_value) . "%'
  ...
  public function get_query_approval_pr_material_management($product, $costcenter, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
  {
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              LEFT JOIN master_customers b ON a.id_customer=b.id_customer,
              (SELECT @row:=0) r
            WHERE 1=1 AND a.category in ('pr material','base on production') AND a.reject_status = '0' AND a.booking_date IS NOT NULL AND z.status_app = 'N' AND a.app_post = '3' " . $costcenter_where . " " . $product_where . " AND (
              b.nm_customer LIKE '%" . $this->db->escape_like_str($like_value) . "%'
              OR a.so_number LIKE '%" . $this->db->escape_like_str($like_value) . "%'
              OR a.project LIKE '%" . $this->db->escape_like_str($like_value) . "%'
              OR a.no_pr LIKE '%" . $this->db->escape_like_str($like_value) . "%'
