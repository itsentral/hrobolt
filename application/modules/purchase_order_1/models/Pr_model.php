...
class Pr_model extends BF_Model
{
	...
	{
		$this->db->select('a.*, b.nm_karyawan AS nama_karyawan, c.nama AS nama_department');
		$this->db->from('tr_purchase_request a');
		$this->db->join('ms_employee b', 'b.id = a.requestor');
		$this->db->join('ms_department c', 'c.id = a.departement_requestor');
		$this->db->where('a.status_po','OPN');
