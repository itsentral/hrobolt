...
class Bentuk_model extends BF_Model
{
    ...
    function generate_id($kode='') {
      $query = $this->db->query("SELECT MAX(id_time) as max_id FROM cycletime_header");
      $row = $query->row_array();
      $thn = date('y');
      $max_id = $row['max_id'];
	...
	public function get_data_category1(){
		...
		public function get_dataindex(){
		$this->db->select('a.*, b.nama_costcenter as nama_costcenter');
		$this->db->from('cycletime_header a');
		$this->db->join('ms_costcenter b','b.id_costcenter=a.costcenter');
		$this->db->where('a.deleted','0');
		$query = $this->db->get();		
		return $query->result();
	}
