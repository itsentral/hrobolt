...
//		if($datauser) $departemen=$datauser->departemen;
        $data = $this->Pengajuan_rutin_model->GetPengajuanRutin(array('status'=>0));
    ...
    public function save_data(){
        ...
        $accname		= $this->input->post("accname");
			...
			$config['allowed_types'] = 'gif|jpg|png|jpeg|bmp|pdf|docx|jfif';
			...
			for ($x = 0; $x < count($detail_id); $x++) {
				...
				if($detail_id[$x]!='') {
					...
					if($nilai[$x]>0) {
						...
						$this->All_model->dataSave('tr_pengajuan_rutin_detail',$data);
					}
				}
			}
