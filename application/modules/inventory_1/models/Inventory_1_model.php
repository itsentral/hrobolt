...
class Inventory_1_model extends BF_Model
{
    ...
    {
        $query =  $this->db->query("UPDATE app_parameter SET VALUE=RIGHT(CONCAT('0',CAST(VALUE AS UNSIGNED)+1),1) WHERE CODE='JP'");
        $seq = $this->db->query("SELECT value FROM app_parameter WHERE CODE='JP'")->row();
        $data = $seq->value;
    ...
    function getById($id)
    {
       return $this->db->get_where('ms_inventory_type',array('id' => $id))->row_array();
    }
}
