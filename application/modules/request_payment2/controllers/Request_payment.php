...
//		$m 			= date_format(date_create($date), 'm');
	...
	public function save_payment()
	{
		...
		$pph_coa		= $this->input->post("pph_coa");
		...
		if (!empty($status)) {
			foreach ($status as $keys => $val) {
				if ($bank_nilai[$keys] <> 0) {
					...
					if ($pph_nilai[$keys] > 0) {
						$det_Jurnaltes1[] = array(
						'nomor' => $nomor_jurnal, 'tanggal' => $payment_date, 'tipe' => 'BUK', 'no_perkiraan' => $pph_coa, 'keterangan' => $keterangan[$keys], 'no_request' => $no_doc[$keys], 'debet' => ($pph_nilai[$keys] < 0 ? ($pph_nilai[$keys] * -1) : 0), 'kredit' => ($pph_nilai[$keys] >= 0 ? $pph_nilai[$keys] : 0), 'no_reff' =>  $no_doc[$keys], 'jenis_jurnal' => $jenis_jurnal, 'nocust' => $nama[$keys]
						);
					}
				}
			}
