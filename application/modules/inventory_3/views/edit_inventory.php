...
<div class="box box-primary">
	...
	<div class="box-body">
		<form id="data_form">
		<input type="hidden" name="id_inventory" id="id_inventory" value='<?= $inven->id ?>'>
		<div class="row">
			<div class="col-md-12">
				<div class="form-group row">
					<div class="col-md-4">
					 <label for="inventory_1">Kategori Produk</label>
					</div>
					<div class="col-md-8">
					  <select id="inventory_1"  name="inventory_1"  class="form-control select" required>
						<option value="">-- Inventory --</option>
						<?php foreach ($results['lvl1'] as $lvl1){
						$select = $inven->id_type == $lvl1->id ? 'selected' : '';
						?>
				...
				<div class="form-group row">
					<div class="col-md-4">
					 <label for="inventory_2" >Brand Produk</label>
					</div>
					<div class="col-md-8">
					  <select id="inventory_2" name="inventory_2" class="form-control select" required>
						<option value="">-- Inventory --</option>
						<?php foreach ($results['lvl2'] as $lvl2){
						$select = $inven->id_category1 == $lvl2->id ? 'selected' : '';
						?>
						<option value="<?= $lvl2->id?>" <?= $select ?>><?= $lvl2->nama?></option>
						<?php } ?>
					  </select>
					</div>
				</div>
				<div class="form-group row">
					...
					<div class="col-md-8">
					  	<input type="text" class="form-control" id="nm_inventory" required name="nm_inventory" placeholder="Tipe Produk" value="<?= $inven->nama ?>">
					</div>
				</div>
				<div class="row">
					<div class="col-md-12">
						<div class="form-group row">
							<div class="col-md-4">
								<label for="">Kode SKU Produk</label>
							</div>
							<div class="col-md-8">
								<input type="text" class="form-control" id="" required name="kode_sku" placeholder="Kode SKU Produk" value="<?= $inven->sku_code ?>">
							</div>
						</div>
					</div>
				</div>
	...
	<div class="row">
		<div class="col-md-12">
			<button type="submit" class="btn btn-primary pull-right" name="save" id="save"><i class="fa fa-save"></i> Save</button>
		</div>
	</div>
	</form>
	</div>
