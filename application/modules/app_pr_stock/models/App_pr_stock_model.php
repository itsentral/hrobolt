...
class App_pr_stock_model extends BF_Model
{
  ...
  public function data_side_approval_pr_material()
  {
    ...
    foreach ($query->result_array() as $row) {
      ...
      $nestedData[]  = "<div align='center'>" . strtoupper($row['stock_name']) . "</div>";
      ...
      if (($row['sts_reject1'] !== null || $row['sts_reject2'] !== null || $row['sts_reject3'] !== null) && $row['rejected'] == 1) {
        ...
        elseif ($row['app_1'] !== null && $row['app_2'] == null && $row['app_3'] == null) :
          $page = "approval_cost_control";
          $warna = "blue";
          $sts = "Waiting Approval Cost Control";
        elseif ($row['app_1'] !== null && $row['app_2'] !== null && $row['app_3'] == null) :
          $page = "approval_management";
          $warna = "blue";
      ...
      $approve  = "";
      $view  = "<a href='" . site_url($this->uri->segment(1)) . '/detail_planning/' . $row['so_number'] . "/" . $page . "' class='btn btn-sm btn-warning' title='Detail PR' data-role='qtip'><i class='fa fa-eye'></i></a>";
  ...
  public function get_query_approval_pr_material($tanda, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
  {
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              JOIN material_planning_base_on_produksi_detail z ON a.so_number=z.so_number
              LEFT JOIN accessories d ON d.id = z.id_material
              LEFT JOIN master_customers b ON a.id_customer=b.id_customer,
              (SELECT @row:=0) r
            WHERE 1=1 AND a.category='pr stok' AND a.booking_date IS NOT NULL AND z.status_app = 'N' AND a.rejected IS NULL " . $filter_approval . " AND (
