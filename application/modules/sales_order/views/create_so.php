...
<div class="box box-primary">
    <div class="box-body">
		...
		<div class="col-md-12">
			...
			<!-- <div class='col-sm-6'>
		        <div class='form-group row'>
                    ...
                    <div class='col-md-8'>
						<input type="date" class="form-control" id="tanggal_pengiriman" onkeyup required name="tanggal_pengiriman" value="<?php echo date('m/d/Y')?>" >
                    </div>
			    </div>
		    </div> -->
		    <!-- <div class='col-sm-6'>
        ...
        <?php } ?>
								...
								<div class="table-responsive">
									<table class='table table-bordered table-striped'>
										...
										foreach ($results['detail'] as $dt_spk){$loop++;
											$ppnProduct = $dt_spk->total_harga * 11/100;
											$totalPPn += $ppnProduct;
											$totalHarga += $dt_spk->total_harga;
											...
											<tr id='tr_$loop'>
												<td>$loop</td>
												<td>
													<select id='used_no_surat_$loop' name='dt[$loop][product_id]' data-no='$loop' onchange='CariDetail($loop)' class='form-control select' required disabled>
														<option value=''>-Pilih-</option>";	
														foreach($material as $produk){
															$select = $dt_spk->id_product == $produk->id ? 'selected' : '';
															$produknama = substr($produk->nama_marketplace, 0, 50);		
															echo"<option value='$produk->id' $select>$produknama</option>";
														}
											echo	"</select>
												</td>
										...
										$totalHargaWithPPn = $totalHarga + $totalPPn;
										...
										<tfoot>
											<tr>
												...
												<th width='7%'></th>                                            
												<th width='7%'><input type='text' class='form-control totalproduk' id='totalproduk' value="<?= number_format($totalHarga, 2, ",", ".") ?>" name='totalproduk' readonly  ></th>										
											...
											<tr>
												...
												<th width='7%'><input type='text' class='form-control totalppn' id='totalppn' value="<?= number_format($totalPPn, 2, ",", ".") ?>"  name='totalppn'  readonly ></th>										
											...
											<th width='7%'></th>                                            
                                            <th width='7%'><input type='text' class='form-control grandtotal' id='grandtotal' value="<?= number_format($totalHargaWithPPn, 2, ",", ".") ?>"  name='grandtotal'  readonly ></th>										
					...
					-->
					<a class="btn btn-danger btn-sm" href="<?= base_url('/penawaran/index') ?>"  title="Edit">Kembali</a>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'sales_order/SaveNewSalesOrder';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
		...
		$('#simpan-com2').click(function(e){
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						var formData 	=new FormData($('#data-form')[0]);
						var baseurl=siteurl+'sales_order/SaveDealNewSalesOrder';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
