...
class Sales_order_model extends BF_Model
{
	...
	function BuatNomor($tanggal) 
  {
      ...
      // month(tgl_so)='$bulan' and 
      $query = $this->db->query("SELECT MAX(no_surat) as max_id FROM tr_sales_order WHERE Year(tgl_so)='$tahun'");
      $row = $query->row_array();
      $max_id = $row['max_id'];
      $max_id1 =(int) substr($max_id,0,3);
      $counter = $max_id1 +1;
      $idcust = sprintf("%03s",$counter)."/SJT/SO/".$romawi."/".$tahun;
  ...
  {
		$this->db->select('a.*, b.nm_customer as name_customer, c.grand_total as total_penawaran, c.no_surat as nomor_penawaran, c.pic_customer as pic, c.tgl_penawaran, d.nm_lengkap');
  ...
  {
		$this->db->select('a.*, b.nm_customer as name_customer, a.pic_customer AS pic, c.grand_total as total_penawaran, c.no_surat as nomor_penawaran, c.tgl_penawaran');
		$this->db->from('tr_sales_order a');
		$this->db->join('master_customers b','b.id_customer=a.id_customer');
    $this->db->join('tr_penawaran c','c.no_penawaran=a.no_penawaran');
    $where = "a.no_so = '$noso'";
