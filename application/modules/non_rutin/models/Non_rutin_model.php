...
class Non_rutin_model extends BF_Model
{
    ...
    public function get_data_json_non_rutin()
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            $list_barang    = $this->db->get_where('non_rutin_planning_detail', array('no_pengajuan' => $row['no_pengajuan']))->result_array();
            $arr_nmbarang = array();
            $arr_spec = array();
            $arr_qty = array();
            $arr_tanggal = array();
            ...
            $view        = "<a href='" . base_url('non_rutin/form/view/1/' . $row['no_pengajuan']) . "' class='btn btn-sm btn-warning' title='View' data-role='qtip'><i class='fa fa-eye'></i></a>";
            $edit        = "";
            $approve    = "";
            $cancel        = "";
            ...
            if ($tanda <> 'approval') {
                ...
                if ($row['sts_app'] == 'N' || $row['sts_app'] == '') {
                    $edit    = "&nbsp;<a href='" . base_url('non_rutin/form/edit/1/' . $row['no_pengajuan']) . "' class='btn btn-sm btn-primary' title='Edit' data-role='qtip'><i class='fa fa-edit'></i></a>";
                }
                // }
            }
    ...
    public function get_data_json_non_rutin_approval_head()
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            $nestedData[]    = "<div align='left'><span class='badge' style='background-color: " . $warna . ";'>" . $sts . "</span></div>";
            $view        = "<a href='" . base_url('non_rutin/form/2/'. $row['no_pengajuan']) . "' class='btn btn-sm btn-warning' title='View' data-role='qtip'><i class='fa fa-eye'></i></a>";
            $edit        = "";
            $approve    = "";
            $cancel        = "";
            $print    = "&nbsp;<a href='" . base_url('non_rutin/print_pengajuan_non_rutin/' . $row['no_pengajuan']) . "' target='_blank' class='btn btn-sm btn-success' title='Print'><i class='fa fa-print'></i></a>";
            ...
            $approve = '';
            if ($ENABLE_MANAGE) {
                $approve    = "&nbsp;<a href='" . base_url('non_rutin/form/edit/1/'. $row['no_pengajuan']) . "' class='btn btn-sm btn-info' title='Approve' data-role='qtip'><i class='fa fa-check'></i></a>";
            }
            $nestedData[]    = "<div align='left'>
									" . $view . "
                                    " . $edit . "
    ...
    public function get_data_json_non_rutin_approval_cost_control()
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            $approve = '';
            if ($ENABLE_MANAGE) {
                $approve    = "&nbsp;<a href='" . base_url('non_rutin/form/edit/2/'. $row['no_pengajuan']) . "' class='btn btn-sm btn-info' title='Approve' data-role='qtip'><i class='fa fa-check'></i></a>";
            }
            $nestedData[]    = "<div align='left'>
									" . $view . "
                                    " . $edit . "
    ...
    public function get_data_json_non_rutin_approval_management()
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            $approve = '';
            if ($ENABLE_MANAGE) {
                $approve    = "&nbsp;<a href='" . base_url('non_rutin/form/edit/3/'. $row['no_pengajuan']) . "' class='btn btn-sm btn-info' title='Approve' data-role='qtip'><i class='fa fa-check'></i></a>";
            }
            $nestedData[]    = "<div align='left'>
									" . $view . "
                                    " . $edit . "
    ...
    public function query_data_json_non_rutin_approval_management($tanda, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
    {
        ...
        $sql = "
			SELECT
				...
				b.nama
			FROM
				non_rutin_planning_detail z
				LEFT JOIN non_rutin_planning_header a ON z.no_pengajuan=a.no_pengajuan
				LEFT JOIN ms_department b ON a.id_dept=b.id,
				(SELECT @row:=0) r
		    WHERE 1=1 " . $where . " AND a.status_id = 1 AND a.no_pr IS NULL AND a.app_post = '3'
            AND (
    ...
    public function query_data_json_non_rutin_approval_cost_control($tanda, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
    {
        ...
        $sql = "
			...
			FROM
				...
				LEFT JOIN ms_department b ON a.id_dept=b.id,
				(SELECT @row:=0) r
		    WHERE 1=1 " . $where . " AND a.status_id = 1 AND a.no_pr IS NULL AND a.app_post = '2'
            AND (
				a.no_pengajuan LIKE '%" . $this->db->escape_like_str($like_value) . "%'
				OR a.tanggal LIKE '%" . $this->db->escape_like_str($like_value) . "%'
				OR a.no_pr LIKE '%" . $this->db->escape_like_str($like_value) . "%'
    ...
    public function query_data_json_non_rutin_approval_head($tanda, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
    {
        ...
        $sql = "
			...
			FROM
				...
				LEFT JOIN ms_department b ON a.id_dept=b.id,
				(SELECT @row:=0) r
		    WHERE 1=1 AND a.status_id = 1 AND a.id_dept = '" . $user_dept_id . "' AND a.no_pr IS NULL AND a.rejected IS NULL AND a.app_post IS NULL
            AND (
				a.no_pengajuan LIKE '%" . $this->db->escape_like_str($like_value) . "%'
				OR a.tanggal LIKE '%" . $this->db->escape_like_str($like_value) . "%'
				OR a.no_pr LIKE '%" . $this->db->escape_like_str($like_value) . "%'
