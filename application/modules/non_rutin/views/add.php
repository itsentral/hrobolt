...
// $tanda 			= (!empty($tanda)) ? 'Update' : 'Insert';
$disabled		= ($tanda == 'add') ? '' : 'disabled';
$disabled2		= ($tanda == 'view') ? 'disabled' : '';
$disabled3		= ($tanda == 'view') ? 'readonly' : '';
		...
		swal({
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					var baseurl = base_url + active_controller + '/form';
					$.ajax({
						url: baseurl,
						type: "POST",
						data: formData,
