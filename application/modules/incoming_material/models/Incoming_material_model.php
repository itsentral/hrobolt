...
class Incoming_material_model extends BF_Model
{
    ...
    public function query_data_json_incoming_material($no_po, $gudang, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
    {
        ...
        $sql = "
            SELECT
                a.*, e.tanggal as incoming_date, b.name_suplier as nama_supplier, c.nm_lengkap, IF(SUM(d.qty_order) IS NULL, 0, SUM(d.qty_order)) as sum_material, e.kode_trans, e.no_ipp
            FROM
                tr_purchase_order a
                LEFT JOIN master_supplier b ON b.code_supplier = a.id_suplier
                LEFT JOIN tr_incoming_check e ON e.no_ipp LIKE CONCAT('%',a.no_po,'%')
            ...
            WHERE
                1=1 AND (SELECT IF(SUM(aa.qty_order) IS NULL, 0, SUM(aa.qty_order)) FROM tr_incoming_check_detail aa WHERE aa.kode_trans LIKE CONCAT('%',e.kode_trans,'%')) > 0 AND (
                    ...
                    b.name_suplier LIKE '%" . $this->db->escape_like_str($like_value) . "%' OR
    ...
    public function modal_detail_adjustment()
    {
        ...
        $get_file_incoming = $this->db->select('file_incoming_material')->get_where('tr_incoming_check', ['kode_trans' => $no_po])->row();

        $data = array(
            'result'     => $result,
            'checked'     => $result_header[0]->checked,
            'no_po' => $result_header[0]->no_po,
            'no_surat' => $result_header[0]->no_surat,
