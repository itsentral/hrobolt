...
class Incoming_material extends Admin_Controller
{
    ...
    protected $viewPermission     = 'Gudang_Pusat_Incoming.View';
    protected $addPermission      = 'Gudang_Pusat_Incoming.Add';
    protected $managePermission = 'Gudang_Pusat_Incoming.Manage';
    protected $deletePermission = 'Gudang_Pusat_Incoming.Delete';
    ...
    public function index()
    {
        ...
        $list_supplier = $this->db->select('code_supplier, name_suplier')->get_where('master_supplier', ['deleted_by' => null])->result();
    ...
    public function incoming_list_po()
    {
        ...
        $no_po = $this->db->query("
            SELECT a.no_po, a.no_surat, a.status, 'PO' as ket_,b.name_suplier AS nm_supplier FROM tr_purchase_order a LEFT JOIN master_supplier b ON b.code_supplier = a.id_suplier WHERE a.status = '2' AND a.tipe IS NULL AND a.id_suplier = '".$kode_supplier."' AND (SELECT IF(SUM(aa.qty_oke + aa.qty_ng) IS NULL, 0, SUM(aa.qty_oke + aa.qty_ng)) FROM tr_checked_incoming_detail aa WHERE aa.no_ipp = a.no_po) < (SELECT SUM(ab.qty) FROM dt_trans_po ab WHERE ab.no_po = a.no_po) AND (SELECT COUNT(ac.id) FROM dt_trans_po ac JOIN new_inventory_4 ca ON ca.code_lv4 = ac.idmaterial WHERE ac.no_po = a.no_po AND ac.idmaterial <> '') > 0 ORDER BY a.no_po ASC
            ")
            ->result();
        ...
        foreach($no_po as $item) {
            ...
            $get_no_pr = $this->db->query("
                ...
                FROM
                    dt_trans_po a
                    JOIN tr_purchase_order b ON b.no_po = a.no_po
                    JOIN non_rutin_planning_detail c ON c.id = a.idpr
                WHERE
                    b.no_surat = '".$item->no_surat."' AND
                    a.tipe = 'pr depart'
                GROUP BY c.no_pr
