...
class Packing_model extends BF_Model{

    public function __construct()
    {
        parent::__construct();

		$this->ENABLE_ADD     = has_permission('Packing.Add');
		$this->ENABLE_MANAGE  = has_permission('Packing.Manage');
		$this->ENABLE_VIEW    = has_permission('Packing.View');
		$this->ENABLE_DELETE  = has_permission('Packing.Delete');
    }
  	...
  	public function get_data_group($table,$where_field='',$where_value='',$where_group='')
	{
  	...
  	public function get_json_packing()
	{
		$controller			= ucfirst(strtolower($this->uri->segment(1)));
		// $Arr_Akses			= getAcccesmenu($controller);
		$requestData		= $_REQUEST;
		$fetch					= $this->get_query_json_packing(
	...
	public function get_query_json_packing($id_category, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$WHERE = '';
		if($id_category > 0){
