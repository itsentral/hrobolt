...
class Penawaran_model extends BF_Model
{
    ...
    function BuatNomor($tgl) 
    {
        ...
        $query = $this->db->query("SELECT MAX(no_surat) as max_id FROM tr_penawaran WHERE Year(tgl_penawaran)='$tahun'");
        $row = $query->row_array();
        
        $max_id = $row['max_id'];
        $max_id1 =(int) substr($max_id,0,3);
        $counter = $max_id1 +1;
        $idcust = sprintf("%03s",$counter)."/SJT/HRB/".$romawi."/".$tahun;
    ...
    {
      $this->db->select('a.*, b.nm_customer as name_customer');
    ...
    {
      // $this->db->select('a.no_surat, a.nama_sales, a.nilai_penawaran, a.tgl_penawaran, a.revisi,a.no_penawaran,a.status, b.nm_customer as name_customer');
