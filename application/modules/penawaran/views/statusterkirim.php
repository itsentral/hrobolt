		...
		<div class="col-sm-12">
            <div class="col-sm-6">
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        <select id="id_customer" name="id_customer" class="form-control select" onchange="get_customer()" readonly required>
		    ...
		    <div class='col-sm-6'>
		        <div class='form-group row'>
			        ...
			        <div class='col-md-8' id="email_slot">
				        <input type='email' class='form-control'  value="<?= $hd->email_customer?>" readonly id='email_customer' required name='email_customer' >
			        </div>
		        </div>
		    </div>
		    <div class='col-sm-6'>
		...
		<div class="col-md-12">
		    <div class='col-sm-6'>
		        <div class='form-group row'>
                    ...
                    <div class='col-md-8'>
                        <select id="top" name="top" class="form-control select" readonly required>
                            <option value="">--Pilih--</option>
                                <?php foreach ($results['top'] as $top){
                                $select2 = $top->id == $hd->top ? 'selected' : '';	?>
                            <option value="<?= $top->id?>" <?= $select2 ?>><?= strtoupper(strtolower($top->name))?></option>
                                <?php } ?>
                        </select>
                    </div>
			    </div>
							...
							<table class='table table-bordered table-striped'>
									...
									foreach ($results['detail'] as $dt_spk){$loop++; 
                                        ...
                                        <tr id='tr_$loop'>
                                            <td>$loop</td>
                                            <td>
                                                <select id='used_no_surat_$loop' name='dt[$loop][no_surat]' data-no='$loop' onchange='CariDetail($loop)' class='form-control select' readonly required>
                                                    <option value=''>-Pilih-</option>";	
                                        ...
                                        echo	"</select>
                                            ...
                                            <td id='qty_$loop'><input type='text' value='$dt_spk->qty' class='form-control input-sm' id='used_qty_$loop' readonly required name='dt[$loop][qty]' onblur='HitungTotal($loop)'></td>
                                            <td id='harga_satuan_$loop'><input type='text' value='$dt_spk->harga_satuan' class='form-control input-sm' readonly id='used_harga_satuan_$loop' required name='dt[$loop][harga_satuan]'></td>
                                            <td id='diskon_$loop'><input type='text' value='$dt_spk->diskon' class='form-control'  id='used_diskon_$loop' readonly required name='dt[$loop][diskon]' onblur='HitungTotal($loop)'></td>
                                            <td id='nilai_diskon_$loop' hidden><input type='text' value='$dt_spk->nilai_diskon' class='form-control' readonly  id='used_nilai_diskon_$loop' required name='dt[$loop][nilai_diskon]'></td>
                                            <td id='total_harga_$loop'><input type='text' value='$dt_spk->total_harga' class='form-control input-sm total' readonly id='used_total_harga_$loop' required name='dt[$loop][total_harga]' readonly></td>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'penawaran/SaveStatusTerkirim';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
