...
class Po_aset extends Admin_Controller {
    ...
    public function save_data(){
		...
		if($tipe_pr=='PP'){
			$supplier_text	= $this->input->post("vendor_pp");
			$nilai_pengajuan= $this->input->post("nilai_pp");
			$notes			= $this->input->post("note_pp");
		}
		if($tipe_pr=='KASBON'){
		...
		if($tipe_pr=='PO'){
			$supplier_text		= $this->input->post("vendor_po3");
			$nilai_pengajuan= $nilai_pr;
			$notes			= $description;
		}
	...
	function create_pp($data){
			...
			if ($qty_inspect > 0){
				...
				$datajurnal1  	 = $this->Acc_model->GetTemplateJurnal($kodejurnal1);
						...
						$nilai_barang= ($nilai_pp+$nilai_ppn)/$qty_inspect;
						
						$datainsert =  array(
						'no_po'=>$nomordoc,
						'no_pr'=>$data['no_pr'],
						'tgl_po'=>$data['tgl_pr'],
						'vendor1_id'=>$vendor_pp,
						'vendor2_id'=>'',
						'vendortext_id'=>'',
						'harga_satuan'=>$nilai_barang,
						'qty' => $qty_inspect,
						'created_by'=> $this->auth->user_id(),
						'created_on'=>date("Y-m-d h:i:s"),
						'terima_barang'=>1,
                     ...
                     $this->identitas_model->DataSave('tr_depresiasi',$datainsert);
...
// hanya terima baran saja
				...
				$search_po  = $this->db->query("SELECT * FROM tr_po_aset WHERE no_po='$no_po'")->row();
		       		$harga_satuan = $search_po->total_nilai_po/$search_po->qty;
				    $datainsert =  array(
						'no_po'=>$no_po,
						'no_pr'=>$search_po->no_pr,
						'tgl_po'=>$tgl_periksa,
						'vendor1_id'=>$search_po->vendor1_id,
						'vendor2_id'=>$search_po->vendor2_id,
						'vendortext_id'=>$search_po->vendortext_id,
						'harga_satuan'=>$harga_satuan,
						'qty' => $search_po->qty,
						'created_by'=> $this->auth->user_id(),
						'created_on'=>date("Y-m-d h:i:s"),
...
//agus update
		...
		$datajurnal  	 = $this->Acc_model->GetTemplateJurnal($kodejurnal);
					...
					foreach($datajurnal AS $record){
						...
						if($tipe_bayar == 1){
						$update_coa	 = "UPDATE tr_po_aset SET coa_um ='$uang_muka' WHERE no_po  = '$no_po' ";
		                $this->db->query($update_coa);
						}
