		...
		<form id="data-form" method="post" autocomplete="off"><br>
					...
					<select name='supplier' id='supplier' class='form-control input-sm pilih_supplier chosen-select'>
						...
						if (!empty($listSupplier)) {
							...
							foreach ($listSupplier as $item) {
								echo "<option value='" . $item->code_supplier . "'>" . $item->name_suplier . "</option>";
							}
						} else {
			...
			<div class="form-group row">
				...
				<div class="col-md-2 text-bold">
					<button type="button" class="btn btn-success" id="btn-barcode">Scan Barcode</button>
				</div>
				<div class="col-md-4">
					<input type="text" onkeyup="return scanbarcode(this)" placeholder="Your Result Scan Here..." class="form-control" id="barcode-scanner" />
				</div>
			</div>
			<hr>
						...
						<table class="table table-striped table-bordered table-hover table-condensed" width="100%">
							<thead>
								<tr class='bg-blue'>
									...
									<th class="text-center" width='8%'>Qty PO (Set)</th>
			...
			<div class="form-group row">
				<div class="col-md-6">
					...
					<button type="button" class="btn btn-info" id="btn-draft" style='margin-left:5px;' name="back" id="back">Make Draft</button>
				</div>
			</div>
		</form>
	</div>
...
<script type="text/javascript">
	...
	function scanbarcode(elem) {
		if (event.keyCode == 13 || event.keyCode == 9) {
			...
			value = document.getElementById("barcode-scanner").value.trim();
			...
			$.ajax({
				...
				url: base_url + active_controller + '/getDataStockbyBarcode',
				success: function(result) {
					if (result.code == 200) {
						$("#qty_in_stok_incoming" + result.data.id_material).focus();
					} else {
	...
	$(document).ready(function() {
		...
		$('#btn-draft').click(function(e) {
			...
			swal({
					title: "Are you sure to Make this Draft?",
				...
				function(isConfirm) {
					if (isConfirm) {
						...
						var baseurl = siteurl + active_controller + '/draft';
						$.ajax({
							url: baseurl,
							type: "POST",
							data: formData,
