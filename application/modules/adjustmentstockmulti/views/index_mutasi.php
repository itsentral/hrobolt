...
<div class="nav-tabs-supplier">
  <ul class="nav nav-tabs">
	<li class="active"><a href="#history" data-toggle="tab" aria-expanded="true">History Mutasi Stock</a></li>
    <li ><a href="#adjust" data-toggle="tab" aria-expanded="true">Add Mutasi</a></li>
  </ul>
 </div> 
...
<div class="box">
	...
	<div class="box-body">
		...
		<?php if(empty($results)){
			...
			$idgudang =$record->id_gudang;			
			$gudangasal 		= $this->db->query("SELECT * FROM ms_gudang WHERE id_gudang='$idgudang'")->row();	
		...
		<tr>
			...
			<td><?= $record->nama?></td>
			<td ><?= $gudangasal->nama_gudang ?></td>
			<td><?= number_format($record->qty,2)?></td>
			<td><?= number_format($record->qty_free,2)?></td>
			<td style="padding-left:20px"> 
			<?php if($ENABLE_VIEW) : ?>
				<a class="btn btn-primary btn-sm edit" href="javascript:void(0)" title="Add Mutasi" data-id_inventory3="<?=$record->id_category3?>" data-id_stock="<?=$record->id_stock?>" data-id_gudang="<?=$record->id_gudang?>"><i class="fa fa-edit"></i></a>
			<?php endif; ?>
			</td>
...
<script type="text/javascript">
	...
	$(document).on('click', '.edit', function(e){
		...
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Mutasi Stok</b>");
		$.ajax({
			type:'POST',
			url:siteurl+'adjustmentstockmulti/MutasiNow/'+id+'/'+idstock+'/'+idgudang+'/'+'index_retur',
