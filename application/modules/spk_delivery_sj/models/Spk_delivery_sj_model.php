...
class Spk_delivery_sj_model extends BF_Model
{
  ...
  public function data_side_spk_material()
  {
    ...
    foreach ($query->result_array() as $row) {
      ...
      $nestedData[]	= "<div align='left'>".strtoupper($row['no_so_surat'])."</div>";
  ...
  public function get_query_json_spk_material($sales_order, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
  {
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              (SELECT @row:=0) r
            WHERE z.deleted_date IS NULL AND (
  ...
  function BuatNomor($tgl) 
    {
        ...
        $query = $this->db->query("SELECT MAX(no_surat_jalan) as max_id FROM spk_delivery WHERE Year(created_date)='$tahun'");
        $row = $query->row_array();
        
        $max_id = $row['max_id'];
        $max_id1 =(int) substr($max_id,0,3);
        $counter = $max_id1 +1;
        $idcust = sprintf("%03s",$counter)."/SJT/DO/".$romawi."/".$tahun;
