...
class Purchase_order extends Admin_Controller
{
	...
	public function index()
	{
		...
		FROM 
			...
			LEFT JOIN non_rutin_planning_detail e ON e.id = c.idpr AND c.tipe = 'pr depart'
			LEFT JOIN non_rutin_planning_header g ON g.no_pengajuan = e.no_pengajuan
			LEFT JOIN master_supplier h ON h.code_supplier = a.id_suplier
	...
	public function edit($no_po)
	{
		...
		$getitemso = $this->db->query("
			...
			SELECT 
				...
				f.code as packing_unit,
				'' as packing_unit2,
				f.code as unit_measure
			FROM
				dt_trans_po a
				JOIN non_rutin_planning_detail e ON e.id = a.idpr
		...
		// $comp	= $this->db->query("select a.*, b.nominal as nominal_harga FROM ms_compotition as a inner join child_history_lme as b on b.id_compotition=a.id_compotition where a.deleted='0' and b.status='0' ")->result();
		$customers = $this->db->get_where('master_customers', ['deleted_by' => null])->result();
		$karyawan = $this->db->get_where('ms_employee', ['deleted_by' => null])->result();
		$mata_uang = $this->db->get_where('mata_uang', ['deleted' => null])->result();
		$list_supplier = $this->db->get_where('master_supplier', ['deleted_by' => null])->result();
	...
	public function SaveNew()
	{
		...
		foreach ($_POST['dt'] as $used) {
			if (isset($used['checked_point'])) {
				...
				if ($valid_qty == 1) {
					...
					if ($used['tipe_pr'] == 'pr depart') {
						$get_data_pr = $this->db->query("SELECT IF(qty IS NOT NULL, qty, 0) AS qty_pr FROM non_rutin_planning_detail WHERE id = '" . $used['idpr'] . "'")->row();
					} else {
						$get_data_pr = $this->db->query("SELECT IF(a.propose_purchase IS NOT NULL, a.propose_purchase, 0) AS qty_pr, b.konversi FROM material_planning_base_on_produksi_detail a JOIN accessories b ON b.id = a.id_material WHERE a.id = '" . $used['idpr'] . "'")->row();
					}

					$qty_all = ($used['qty'] + $get_other_po_brg->other_qty);
					// print($used[idpr].' ('. $used[qty] . ' + '.$get_other_po_brg->other_qty.') - ' . $get_data_pr->qty_pr.'<br>');
					if ($get_other_po_brg->other_qty > ($get_data_pr->qty_pr * $get_data_pr->konversi )) {
						$valid_qty = 0;
					}
				}
		...
		// Untuk PR Departemen
		$this->db->select('*');
		$this->db->from('non_rutin_planning_header');
		$this->db->where_in('no_pengajuan', explode(',', $post['so_number']));
		$countDept = $this->db->get()->row_array();
		if (count($countDept) > 0) {
			$update_rutin_non_planning = $this->db->where_in('no_pengajuan', explode(',', $post['so_number']))->update('non_rutin_planning_header', ['po_number' => $code, 'po_date' => date('Y-m-d')]);
	...
	public function PrintH2()
	{
		...
		$this->auth->restrict($this->managePermission);
		$id = $this->uri->segment(3);
		$data['header'] = $this->db->query("SELECT a.*, a.id_suplier, b.name_suplier as nm_supp, b.address_office as alamat, c.name as country_name, b.telephone as nm_pic, b.telephone as hp, b.email as email_pic, b.fax FROM tr_purchase_order as a LEFT JOIN material_planning_base_on_produksi x ON x.po_number = a.no_po LEFT JOIN master_supplier b ON b.code_supplier = a.id_suplier LEFT JOIN country_all c ON c.iso3 = b.id_negara WHERE a.no_po = '" . $id . "' ")->result();
		...
		if ($data['header'][0]->tipe == 'pr depart') {
			...
			LEFT JOIN non_rutin_planning_detail b ON b.id = a.idpr
			...
			$this->db->join('non_rutin_planning_header b', 'b.id_dept = a.id', 'left');
			$this->db->join('non_rutin_planning_detail c', 'c.no_pengajuan = b.no_pengajuan', 'left');
			...
			$this->db->join('non_rutin_planning_detail b', 'b.no_pengajuan = a.no_pengajuan', 'left');
			...
			$data['detail']  = $this->db->query("SELECT a.*, 
				a.namamaterial as nama, IF(b.code IS NULL OR b.code = '', e.id_stock, b.code) AS code, IF(e.konversi IS NULL, 1, e.konversi) AS konversi, c.code as satuan, d.code as satuan_packing 
				FROM dt_trans_po a 
		...
		$data['data_supplier'] = $this->db->get_where('master_supplier', ['code_supplier' => $data['header'][0]->id_suplier])->row();
	...
	public function proses()
	{
		...
		// return $this->output->set_status_header(200)->set_content_type('application/json')->set_output(json_encode($list_supplier));
